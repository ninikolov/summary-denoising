...
gpus=(${3//,/ })
gpu_count=${#gpus[@]}
# Divide base learning rate by the number of GPUs
lr=$(bc -l <<< "0.0003125/$gpu_count")
echo "Learning rate with $gpu_count GPUs: $lr"
echo

if [ "$model_architecture" == "lstm_tiny" ]; then
    # Train LSTM
    CUDA_VISIBLE_DEVICES=$3 python $FAIRSEQ_PATH/train.py $data_save_dir \
        ...
        --no-progress-bar --skip-invalid-size-inputs-valid-test --max-tokens 8000 $share_embs \
        --no-epoch-checkpoints
elif [ "$model_architecture" == "transformer_vaswani_wmt_en_de_big" ]; then
    # Train Transformer
    CUDA_VISIBLE_DEVICES=$3 python $FAIRSEQ_PATH/train.py $data_save_dir \
        --optimizer adam --adam-betas '(0.9, 0.98)' --clip-norm 0.0 --lr-scheduler inverse_sqrt \
        --warmup-init-lr 1e-07 --warmup-updates 4000 --lr 0.0003125 --min-lr 1e-09 --weight-decay 0.0 \
        --criterion label_smoothed_cross_entropy --label-smoothing 0.1 \
        --arch $model_architecture --save-dir $model_architecture --no-progress-bar \
        --skip-invalid-size-inputs-valid-test --max-tokens 2000 $share_embs \
        --no-epoch-checkpoints
fi
